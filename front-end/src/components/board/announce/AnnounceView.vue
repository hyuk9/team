        ...
        <div class="card mb-5" v-if="showInsertComment">
          <div class="card-body input-group">
            ...
            ><textarea
              class="form-control"
              row="1"
              v-model="aidComment.content"
            ></textarea>
            <button
              type="button"
              class="btn btn-danger"
              @click="saveAidComment"
            >
              등록
            </button>
          </div>
...
export default {
  ...
  methods: {
    ...
    getCommentByAid() {
      // axios 공통함수 호출
      CommentDataService.getCommentByAid(
        ...
        .then((response) => {
          // springboot 결과를 리턴함(부서 객체)
          const { comment, totalItems } = response.data; // springboot 의 전송된 맵 정보
        ...
        // 실패하면 .catch() 에러메세지가 리턴됨
        .catch((e) => {
          console.log("공지사항 번호에 해당하는 댓글 정보 조회요청 실패 : ", e);
        });
    },
    ...
    saveAidComment() {
      if (this.currentUser.id != null) {
        let data = {
          ...
          content: this.aidComment.content,
        };

        CommentDataService.create(data)
          // 성공하면 then() 결과가 전송됨
          .then((response) => {
            this.aidComment.cno = response.data.cno;
  ...
  computed: {
    ...
    showDetailBoard() {
      if (this.currentUser && this.currentUser.roles) {
        ...
        return (
          this.currentUser.roles.includes("ROLE_ADMIN") ||
          this.currentUser.id == this.currentAnnounce.id
        );
      }
