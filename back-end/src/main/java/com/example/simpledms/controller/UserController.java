...
//    TODO : Mybatis 컨트롤러 함수

    //  Mybatis 사용
    @GetMapping("/user")
//    @PreAuthorize("hasRole('ADMIN')")
    public ResponseEntity<Object> getUserAll(Criteria criteria) {

        try {

//            페이지 변수 저장
            List<UserRoleCDto> userList = userService.findAllByUsernameContaining(criteria);

            Map<String, Object> response = new HashMap<>();
            response.put("user", userList);
            response.put("currentPage", criteria.getPage());
            response.put("totalItems", criteria.getTotalItems());
            response.put("totalPages", criteria.getTotalPages()+1);
